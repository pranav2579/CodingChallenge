...
public class RepeatAndMissing {
        ...
        public static ArrayList<Integer> repeatedNumber(final List<Integer> a) {
            ...
            //finding b-a;
            int normalSum = 0, currentSum = 0,difference = 0;
            for (int i = 1; i <= a.size(); i++) {
                normalSum += i;
                currentSum += a.get(i - 1);
            }

            difference = currentSum - normalSum;
            //boolean largestRepeated = false;
            if(difference < 0)
            {
                difference *= -1;
                //largestRepeated = true;
            }

            //finding b/a
            BigInteger CurrentFactorial = new BigInteger("1");
            for (int i = 0; i < a.size(); i++) {
                    BigInteger temp = new BigInteger(a.get(i).toString());
                    CurrentFactorial = CurrentFactorial.multiply(temp);
                }
            BigInteger NormalFactorial = new BigInteger("1");
            for (int i = 1; i <= a.size(); i++) {
                BigInteger temp = new BigInteger(Integer.toString(i));
                NormalFactorial = NormalFactorial.multiply(temp);
            }


            BigInteger div;
            //if(!largestRepeated){
                 div = CurrentFactorial.divide(NormalFactorial);
            //}
            //else{
                 //div = NormalFactorial.divide(CurrentFactorial);
            //}
            int division = div.intValue();

            //calculations of a and b
            int first = difference/(division - 1);
